    ...
    <section class="panel table-panel">
      <div class="table-head">
        <div class="search">
          ...
          <input type="text" v-model="searchQuery" @input="onSearchInput" placeholder="Cari lokasi cabang ...">
        </div>
...
.search{
  ...
  padding:10px 16px;border-radius:10px;min-width:280px;
}
...
.icon-btn-solid{
  width:44px;height:44px;border-radius:10px;background:var(--green-900);border:none;
  display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;
  transition:background .15s ease;
}
.icon-btn-solid:hover{background:#0f2b22;}
.icon-btn-solid svg,.icon-btn-solid .iconify{color:#fff;}

table{width:100%;border-collapse:collapse;}
thead tr{background:var(--green-900);}
thead th{color:#dfe9e3;font-size:11.5px;font-weight:700;letter-spacing:.06em;text-align:left;padding:14px 24px;text-transform:uppercase;}
tbody td{padding:18px 24px;font-size:15px;border-bottom:1px solid var(--line);vertical-align:middle;color:var(--ink);}
tbody tr:last-child td{border-bottom:none;}
.empty-cell{text-align:center;color:var(--ink-soft);padding:32px;}
.loc-name{font-weight:700;font-size:15px;}
.loc-id{font-size:12px;color:var(--ink-soft);margin-top:2px;}
.badge{
  display:inline-flex;align-items:center;font-size:12.5px;font-weight:700;
  padding:5px 14px;border-radius:20px;
}
.badge.aktif{background:var(--mint-bg);color:var(--mint-text);}
.badge.nonaktif{background:var(--red-bg);color:var(--red);}

.table-footer{
  display:flex;justify-content:space-between;align-items:center;
  padding:16px 24px;font-size:13px;color:var(--ink-soft);border-top:1px solid var(--line);
}
...
.pager button svg,.pager button .iconify{color:var(--ink-soft);}
.pager button:disabled{opacity:.4;cursor:not-allowed;}

.add-btn-row{display:flex;justify-content:flex-end;margin-top:20px;}
.add-btn{
  display:flex;align-items:center;gap:8px;
  background:var(--green-900);color:#fff;border:none;
  padding:14px 22px;border-radius:12px;font-size:14.5px;font-weight:700;cursor:pointer;
  transition:background .15s ease;
}
.add-btn:hover{background:#0f2b22;}
.add-btn svg,.add-btn .iconify{color:#fff;}

@media (max-width:700px){
  .table-head{justify-content:stretch;}
